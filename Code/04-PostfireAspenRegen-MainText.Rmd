...
pacman::p_load(
  ...
  officer, # word documents --> useful for formating
  flextable, # tables
  ggplot2, # figures
  patchwork, # multiple figures
  readxl, # read excel files
  ggcorrplot, # plotting
  ggbreak, # axis break for ggplots
  glmmTMB, # ZINB
...
stocking.plot <- seedlingdensity.dat  %>% 
  mutate_if(is.numeric, ~ if_else(. < 1, 0, 1)) %>% 
  group_by(Site) %>% 
  summarize_at(.vars=vars(ABLA:Conifer), .fun=sum)

# Calculate number of subplots with multiple seedlings per plot
mstocking.plot <- seedlingdensity.dat %>% 
  mutate_if(is.numeric, ~ if_else(. < 2, 0, 1)) %>% 
  group_by(Site) %>% 
  summarize_at(.vars=vars(ABLA:Conifer), .fun=sum)

# Calculate number of subplots with aspen and conifer seedlings 
castocking.plot <- seedlingdensity.dat %>% 
  mutate_if(is.numeric, ~ if_else(. < 1, 0, 1)) %>% 
  mutate(combo= ifelse(POTR5.seedling >0 & Conifer >0, 1,0)) %>%
  group_by(Site) %>% 
  summarize(conifer=sum(Conifer), aspen =sum(POTR5.seedling), aspenconifer=sum(combo))
...
Pre-fire forest composition and structure within the burn scar of the Cameron Peak Fire scar also varied with elevation. Lower elevation zones below 2800 m were characterized by dry and mesic montane forests, dominated by ponderosa pine (*Pinus ponderosa*), with lesser components of Douglas-fir (*Pseudotsuga menziesii*). Moderate and higher elevation stands between approximately 2800 – 3300 m were made up of subalpine forest, dominated by Engelmann spruce (*Picea engelmannii*), subalpine fir (*Abies lasiocarpa*), and lodgepole pine (*Pinus contorta*). Notably, lodgepole pine is the only species capable of producing a canopy seedbank, whereas all other depend upon dispersal of seed from live trees to regenerate post-fire. Thus, this gradient in species composition imprints a pattern of post-fire regeneration traits across the landscape.

The disturbance regimes of forests across the study area are shaped by wildfire, outbreaks of native tree-killing bark beetles, and windthrow [@baker1990SpruceBeetlesFires; @veblen1994DisturbanceRegimeDisturbance]. Notably n the two decades prior to the Cameron Peak Fire, outbreaks of mountain pine beetles (*Dendroctonus ponderosae*), spruce beetles (*Dendroctonus rufipennis)*, and western balsam bark beetles have caused mortality of pine, Engelmann spruce, and subalpine fir, respectively, across about 40 % of the burned area [@rodman2021EffectsBarkBeetle]. This pre-fire tree mortality has likely reduced the availability of viable seed across the burned area [@andrus2021FutureDominanceQuaking; @rhoades2022LimitedSeedViability; @rhoades2025PostfireSeedlingRecruitment].

Only about `r round(study.area.CP %>% pull(aspen) %>% mean()*100)` % of the area burned was dominated by aspen prior to fire, most (\~80%) of which occurred between `r study.area.CP %>% filter(aspen==1) %>% pull(elevation) %>% quantile(0.10) %>% round()` and `r study.area.CP %>% filter(aspen==1) %>% pull(elevation) %>% quantile(0.90) %>% round()` m in elevation. About `r round(study.area.CP %>% filter(aspen==1) %>% group_by(severity) %>% summarise(n=n()) %>% mutate(freq=n/sum(n)) %>% filter(severity==4) %>% pull(freq)*100)` % of the aspen forest burned at high severity, while about `r round(study.area.CP %>% filter(EVT=="Conifer") %>% group_by(severity) %>% summarise(n=n()) %>% mutate(freq=n/sum(n)) %>% filter(severity==4) %>% pull(freq)*100)` % of the conifer forest burned at high severity.

## Study sites

We surveyed `r nrow(site.dat)` sites in the summer of 2022 (Fig. \@ref(fig:FigStudyArea)). Sites were randomly located using geospatial data describing the extent and severity of the Cameron Peak Fire from the Monitoring Trends in Burn Severity (MTBS) Project [-@mtbsproject2024MTBSDataAccess; -@mtbsproject2024MTBSDataAccessa] and the extent of pre-fire aspen cover from Cook et al. [-@cook2024MappingQuakingAspen]. We combined these data sources to identify areas that: (a) burned at high severity and (b) were located \>50 m and \<1000 m away from a pre-fire patch of aspen. Sites were further limited to accessible areas, defined as public land within 1000 m of a road or 500 m away from a trail and no more than 1 km up the trail. To reduce the potential effects of psuedoreplication, all sites were located \>400 meters apart, or about twice as far as the minimum sampling distance used by Kreider and Yocom [-@kreider2021LowdensityAspenSeedling]. During field sampling we confirmed that all constraints were met. Despite the extensive area burned, the accessible area was notably restricted by a limited road network, damage to infrastructure, postfire flooding and forest treatments (e.g., aerial mulching) that occurred concurrently with our sampling. Due to the dynamic nature of the post-fire environment, during the field sampling campaign we daily reviewed information on forest closures and ruled out potential plots that were inaccessible. Nonetheless our sample sites span broad gradients in elevation (`r site.dat %>% pull(elevation) %>% min() %>% round()` - `r site.dat %>% pull(elevation) %>% max() %>% round()` m), mean annual daily temperature (`r site.dat %>% pull(tmean) %>% min() %>% round(1)` - `r site.dat %>% pull(tmean) %>% max() %>% round(1)`°C), and annual total precipitation (`r site.dat %>% pull(ppt) %>% min() %>% round()` - `r site.dat %>% pull(ppt) %>% max() %>% round(0)` mm) (1991-2020 means) (Table S\@ref(tab:Tabstudysites)).

## Field data collection

At each site, we recorded the coordinates, elevation, slope, and aspect. We also recorded the conifer species present prior to fire within a 20 x 20 m plot following methods we have applied previously [@andrus2016FireSeverityUnaffected]. We then established two perpendicular 2 x 50 m transects that intersected at the midpoint [@andrus2021FutureDominanceQuaking]. Along each transect, we collected information on percent herbaceous cover (i.e., the combined cover of grass, forbs, and shrubs) in three 1 x 1 m quadrants located every 20 m along each transect. We tallied the presence of all conifer and aspen seedlings, and aspen root suckers within each transect and recorded their location along the transect within 2 x 2m subplots. Aspen seedlings were differentiated from root suckers using methods outlined by Kreider et al. [-@kreider2020MethodsDistinguishingAspen], which have been demonstrated to be more than 95 % accurate [@kreider2021LowdensityAspenSeedling] and were shown to be effective in our study area [@carter2024OccurrenceDistributionDriving]. We excluded any sites where aspen suckers were observed.

As a proxy for aspen seed availability, we also recorded the distance from plot center to live aspen trees. Where live aspen were not visible in the field, we estimated the distance to live aspen using a GIS. Specifically, we overlaid the MTBS map of fire severity [@mtbsproject2024MTBSDataAccess] on Cook et al.'s [@cook2024MappingQuakingAspen] map of pre-fire aspen occurrence and assumed that any aspen stem that burned at moderate or high severity was likely killed in the fire. We then calculated the distance from plot center to the nearest live aspen pixel in R [@rcoreteam2024LanguageEnvironmentStatistical] using the *terra* package [@hijmans2022TerraSpatialData]. To quantify the distance to the nearest conifer, we manually recorded distance to the nearest live conifer tree viable in 1-m aerial imagery collected in 2021 by the National Agricultural Imagery Program [@aerialphotographyfieldoffice2021NationalAgricultureImagery] using QGIS (version 3.3.8).

To test the hypothesis that the microsite environment influence aspen seedling establishment, we additionally recorded the following microsite conditions: proximity to CWD, substrate, and microtopography for each aspen seedling. We characterized the microtopography as flat, concave, or convex within 2.5 cm of seedling (hereafter ‘small microtopography’) and within 50 cm of seedling (hereafter ‘large microtopography’). We also recorded the presence or absence of small (diameter = 2.5-10 cm) and large (diameter \>10 cm) CWD within 10 cm of seedling. We also collected microsite conditions at 5 m intervals along each transect to serve as a baseline of the relative abundance of each condition across the site.
...
To understand if seedlings were clustered within sites, we quantified the proportion of seedlings that occurred in the same 2 x 2 m subplot with another seedling and the proportion of occupied (\>1 seedling) subplots with multiple seedlings. We summarized data by species, for all conifers combined, and then the co-occurrence of aspen and conifer seedlings. To statistically test for clustering, we then used a Chi-squared goodness-of-fit-test where the null hypothesis was complete spatial randomness (i.e., that seedlings were equally distributed across the subplots). Because clustering is difficult to detect at low densities, we only tested for clustering of seedlings in plots with at least 1000 seedlings per ha (i.e., or on average 2 seedlings per subplot). To quantify statistical significance, we used a bootstrapping approach with 1000 replications. We performed this analysis separately for each site and tested for spatial randomness among aspen seedlings and then among all conifer seedlings combined. We also tested if conifer and aspen seedlings were more likely to co-occur using a Chi-squared goodness-of-fit-test where the null hypothesis was that conifer seedlings were equally likely to occur in subplots with and without aspen seedlings. As above, we performed this analysis separately for each site and used a bootstrapping approach with 1000 replications to determine the statistical significance. All tests were performed in R [@rcoreteam2024LanguageEnvironmentStatistical].

```{r co}
# calculate average number of subplots with seedlings per site when seedlings were present
cotab.n.all <- stocking.plot %>% 
  dplyr::select(Site, POTR5.seedling, PIPO, PSME, PICO, ABLA, PIEN,  Conifer) %>%
  pivot_longer(cols=-Site, values_to="nall")

cotab.n.multi <- mstocking.plot %>% 
  dplyr::select(Site, POTR5.seedling, PIPO, PSME, PICO, ABLA, PIEN,  Conifer) %>%
  pivot_longer(cols=-Site, values_to="nmulti") 

cotab <- full_join(cotab.n.all , cotab.n.multi, by=c("Site","name")) %>% 
  filter(nall>0) %>%
  mutate(prop=nmulti/nall) %>% 
  mutate(name=factor(name, levels=c( "POTR5.seedling", "PIPO", "PSME", "PICO","ABLA", "PIEN",  "Conifer"), labels=c( "quaking\naspen", "ponderosa\npine", "Douglas\nfir", "lodgepole\npine", "subalpine\nfir", "Engelmann\nspruce",  "all\nconifers"), order=TRUE))

p1 <- cotab %>% ggplot(aes(x=name, y=prop))+geom_boxplot()+ stat_summary(fun = mean, geom = "point", shape = 18, size = 3, color = "darkgreen")+ylab("proportion of subplots with multiple\nseedlings per site")+xlab("Species")+theme(axis.text.x = element_text(angle=45, hjust=1))
ggsave(p1,  filename=here("Results", "Figures", "SubCluster.jpg"), width=5, height=3.5, units="in")

cotab <- left_join(cotab, seedlingdensity.plot, by="Site")

p1 <- cotab %>% filter(name=="quaking\naspen") %>% ggplot(aes(x=POTR5.seedling.density, y=prop))+geom_point()+ylab("proportion of subplots with multiple\naspen seedlings per site")+xlab("aspen seedling density (no. per ha)")
p2 <- cotab %>% filter(name=="all\nconifers") %>% ggplot(aes(x=POTR5.seedling.density, y=prop))+geom_point()+ylab("proportion of subplots with multiple\nconifer seedlings per site")+xlab("conifer seedling density (no. per ha)")

ggsave(p1 + p2,  filename=here("Results", "Figures", "SubClusterXdensity.jpg"), width=5, height=3.5, units="in")
...
jpeg(filename=here("Results", "figures", "zinb-MoransI.jpg"), width=5, height=5, units="in", res=300)
testSpatialAutocorrelation(simulationOutput = site.mod.reduced.sim, x = compiled.site.plot$Easting, y= compiled.site.plot$Northing)
dev.off()
...
zinb.coef <- zinb.coef  %>%
  ...
  mutate(Component=c(rep("Count", 2), rep("Binary", 1))) %>% 
  mutate(Predictor=factor(Predictor, levels=c( "Elevation", "HeatLoad", "Herb", "Conifer", "distancepost"), labels=c("Elevation",  "Heat load", "Herbaceous cover", "Conifer seedling density", "Seed-source distance"), ordered=T)) %>%
...
To understand if aspen seedlings preferentially established in different microsite conditions, we followed the approach outlined by Kreider and Yocom [-@kreider2021AspenSeedlingEstablishment]. Briefly, this approach compares the probability that each category of a microsite variable (e.g., concave, convex, or flat microtopography) occurs where seedlings established to the probability that each category occurs in the systematic sample of microsites. To account for the nested nature of the data (i.e., seedlings within sites), we used a mixed effect multinomial logistic regression approach that included a random intercept term of site identity to model the probability that a seedling occurred within each category of a microsite variable. We then modeled the probability that systematically surveyed points occurred in each category of the variable, also using a using a mixed effect multinomial logistic regression that included a random intercept term of site identity. We used these two models to quantify microsite preference following methods outlined by Krieder and Yocom [-@kreider2021AspenSeedlingEstablishment], where:

$$
preference = \frac{P(occurence) - P(availabilty)}{P(availability)}
$$

and *P(occurrence)* is the probability of seedling occurring in a given microsite condition and *P(availability*) is the probability of a given microsite condition is available (i.e., the probability that systematically surveyed points occurred in each category). Here, preference values greater than zero indicate seedlings established in that microsite condition more often than expected based on the systematic sample. Multinomial regression was performed in R [@rcoreteam2024LanguageEnvironmentStatistical] using the *mclogit* package [@mclogit] and results were visualized using the *ggplot2* [@ggplot2] and *ggbreak* [@ggbreak] packages.
...
theme(strip.background.y = element_blank(), strip.text.y = element_blank(), strip.background.x = element_rect(colour = "black"), strip.text.x=element_text(margin = margin(0.05,0,0.05,0, "in")))+scale_color_manual(values=c("black", "#4F81BD"))+theme(legend.position="none", axis.title.x=element_blank(), axis.text.x=element_blank(), ) +scale_y_continuous(breaks=c(-9,0,9,20,25,30),labels=c(-1, 0, 1, 20, 25, 30) )
...
Across sites that were dominated by conifers prior to fire and then severely burned, we found limited seedling regeneration two years following fire (Table \@ref(tab:seedlingdensity)). Aspen seedlings were present at `r (seedlingdensity.tph %>% filter(POTR5.seedling>0) %>% nrow() / nrow(seedlingdensity.tph) *100) %>% round(0)` % of sites, with a mean density of `r seedlingdensity.tph %>% filter(POTR5.seedling>0) %>%  pull(POTR5.seedling) %>% mean() %>% round(0)` seedlings per hectare. Conifer seedlings were present at the same proportion of sites (`r (seedlingdensity.tph %>% filter(Conifer>0) %>% nrow() / nrow(seedlingdensity.tph) *100) %>% round(0)` % of plots), but at a lower density on average when present (Table \@ref(tab:seedlingdensity)).

```{r Tabseedingdensity, tab.cap='The number (n) and percentage of sites with postfire seedlings present and measures of density (seedlings per ha) for sites with seedlings present.', tab.id='seedlingdensity'}
comp1.summary %>% mutate(mean=round(mean, 0), percentage=round(n/34*100,0)) %>% relocate(percentage, .after=n) %>%  qflextable() %>% set_table_properties(layout = "autofit", width=0.5) %>% fontsize(size = 10)
```

Generally, the conifer species present in the postfire community reflected the pre-fire community. Lodgepole pine seedlings were present at `r (comp.presence %>% filter(name=="PICO" &combo=="present") %>% pull(percent)) %>% round()` % of sites where lodgepole was present prior to fire (Fig. \@ref(fig:FigPostFireComp)). In contrast, Engelmann spruce and subalpine fir seedlings were only present at `r comp.presence %>% filter(name=="PIEN" &combo=="present") %>% pull(round(percent))` % and `r comp.presence %>% filter(name=="ABLA" &combo=="present") %>% pull(round(percent))` % of sites where they occurred prior to fire. Ponderosa pine occurred at `r comp.presence %>% filter(name=="PIPO" &combo=="present") %>% pull(round(percent))` % of sites and Douglas fir seedlings were entirely absent postfire; however, the sample size for both species was small (n~ponderosa pine~=`r comp2 %>% group_by(name) %>% summarize(sum=sum(presence)) %>% filter(name=="PIPO") %>% pull(sum)` sites; n~Douglas fir~=`r comp2 %>% group_by(name) %>% summarize(sum=sum(presence)) %>% filter(name=="PSME") %>% pull(sum)` sites). Postfire seedlings of lodgepole pine and Engelmann spruce were both observed at one site where they were absent in the pre-fire community (Fig. \@ref(fig:FigPostFireComp)).

```{r FigPostFireComp, fig.cap="Patterns of postfire seedling presence by species and pre-fire species presence in the overstory.", fig.height=3, fig.width=5, eval=T}
knitr::include_graphics(here("Results", "Figures", "PostFireComposition.jpg"))
```

## Clustering

Across all sites, we found `r ((seedlingdensity.dat %>% filter(POTR5.seedling>1) %>% nrow()) / (seedlingdensity.dat %>% filter(POTR5.seedling>0) %>% nrow()) *100) %>% round()` % of aspen seedlings occurred in subplots with another aspen seedling. Within sites with aspen seedlings, on average `r round((cotab %>% group_by(name) %>% summarize(prop=mean(prop)) %>% filter(name=="quaking\naspen") %>% pull(prop)*100),0)` % of the subplots with aspen seedlings contained multiple aspen seedlings (Fig. S\@ref(tab:FigSubCluster)). Statistical tests of clustering showed that aspen seedlings were significantly (p\<0.05) clustered at all sites with at least 1,000 aspen seedlings per hectare (Table S\@ref(tab:Tabcsraspen)). Clustering of aspen seedlings within subplots had a tendency to occur at sites with higher overall densities (Fig. S\@ref(tab:FigSubClusterXDensity)). Yet at sites with less than 1,000 aspen seedlings per hectare, we found on average `r round(cotab %>% filter(name=="quaking\naspen" & POTR5.seedling.density<1000) %>% pull(prop) %>% mean()*100)`% of subplots with aspen seedlings had multiple seedlings even though only `r round(cotab %>% filter(name=="quaking\naspen" & POTR5.seedling.density<1000) %>% pull(nall) %>% mean())` subplots had seedlings.

We found `r ((seedlingdensity.dat %>% filter(Conifer>1) %>% nrow()) / (seedlingdensity.dat %>% filter(Conifer>0) %>% nrow()) *100) %>% round()` % of conifer seedlings occurred in subplots with another conifer seedling across our sites. We found multiple ponderosa pine seedlings in `r ((seedlingdensity.dat %>% filter(PIPO>1) %>% nrow()) / (seedlingdensity.dat %>% filter(PIPO>0) %>% nrow()) *100) %>% round()` % of subplots with ponderosa pine seedlings, multiple lodgepole pine in `r ((seedlingdensity.dat %>% filter(PICO>1) %>% nrow()) / (seedlingdensity.dat %>% filter(PICO>0) %>% nrow()) *100) %>% round()` % of subplots with lodgepole pine seedlings, multiple Engelmann spruce in `r ((seedlingdensity.dat %>% filter(PIEN>1) %>% nrow()) / (seedlingdensity.dat %>% filter(PIEN>0) %>% nrow()) *100) %>% round()` % of subplots with Engelmann spruce, and multiple subalpine fir in `r ((seedlingdensity.dat %>% filter(ABLA>1) %>% nrow()) / (seedlingdensity.dat %>% filter(ABLA>0) %>% nrow()) *100) %>% round()` % of subplots with subalpine fir. Within sites with conifer seedlings, on average `r round((cotab %>% group_by(name) %>% summarize(prop=mean(prop)) %>% filter(name=="all\nconifers") %>% pull(prop)*100),0)` % of the fifty 2 x 2 m subplots with conifer seedlings contained multiple conifer seedlings (Fig. S\@ref(tab:FigSubCluster)) and clustering of conifer seedlings within subplots occurred across the gradient of conifer seedling density (Fig. S\@ref(tab:FigSubClusterXDensity)). Across the `r csr.res.con %>% nrow()` sites with at least 1,000 conifer seedlings per hectare, conifer seedlings were significantly (p\<0.05) clustered at both sites (Table S\@ref(tab:Tabcsrcon)).

We found `r ((seedlingdensity.dat %>% filter(Conifer>0 & POTR5.seedling>0) %>% nrow()) / (seedlingdensity.dat %>% filter(Conifer>0) %>% nrow()) *100) %>% round()` % of conifer seedlings occurred in subplots with an aspen seedling across our sites. Within sites with both conifer and aspen seedlings, on average `r round(castocking.plot %>% filter(conifer>0 & aspen>0) %>% mutate(x=aspenconifer/conifer*100) %>% pull(x) %>% mean(),0)` % of subplots with conifer seedlings also had aspen seedlings. In the two sites with more than 1000 conifer and aspen seedlings, conifer seedlings were significantly (p\<0.05) more likely to occur in subplots with aspen than expected by complete spatial randomness (Table S\@ref(tab:Tabcsrconaspen)), suggestive of common microsite preferences. At these sites we found most (`r round((seedlingdensity.dat %>% filter(Site==38) %>% filter(Conifer>0 & POTR5.seedling>0) %>% nrow())  / (seedlingdensity.dat %>% filter(Site==38) %>% filter(Conifer>0) %>% nrow()) *100)` - `r round((seedlingdensity.dat %>% filter(Site==25) %>% filter(Conifer>0 & POTR5.seedling>0) %>% nrow())  / (seedlingdensity.dat %>% filter(Site==25) %>% filter(Conifer>0) %>% nrow()) *100)` %) of the subplots with conifer seedlings also had aspen seedlings. Most of this co-occurrence included lodgepole pine; `r seedlingdensity.dat %>% filter(Site==25|Site==38) %>% filter(PICO>0 & POTR5.seedling>0) %>% nrow()` out of `r seedlingdensity.dat %>% filter(Site==25|Site==38) %>% filter(PICO>0) %>% nrow()` subplots had both lodgepole pine and aspen seedlings. However, Engelmann spruce co-occurred with aspen seedlings in 3 of the 7 subplots where spruce seedlings were present.

## Site-level drivers of postfire aspen seedling density

Site-level variability in bioclimatic conditions influenced the density of aspen seedlings observed two-years following the Cameron Peak Fire. Our model of aspen seedling density explained a moderate amount of the variation (pseudo R^~2~^ = `r site.mod.r2`) and correctly predicted `r round(site.mod.01 %>% filter(predict01==1 & POTR5.seedling01==1) %>% pull(prop)*100)` % of the sites where aspen seedlings were observed (`r site.mod.01 %>% filter(predict01==1 & POTR5.seedling01==1) %>% pull(n)` of the `r site.mod.01 %>% filter(POTR5.seedling01==1) %>% pull(n) %>% sum()` sites with aspen seedlings), but only `r round(site.mod.01 %>% filter(predict01==0 & POTR5.seedling01==0) %>% pull(prop)*100)` % of the absences (`r site.mod.01 %>% filter(predict01==0 & POTR5.seedling01==0) %>% pull(n)` of the `r site.mod.01 %>% filter(POTR5.seedling01==0) %>% pull(n) %>% sum()` sites without aspen seedlings). The binary part of the model showed that the absence of aspen seedling tended to occur at lower elevations (standardized β = `r zinb.coef %>% filter(Component=="Binary" & Predictor=="Elevation") %>% pull(Estimate)`; standard error = `r zinb.coef %>% filter(Component=="Binary" & Predictor=="Elevation") %>% pull(4)`; Fig. \@ref(fig:FigDensity)A), however the statistical support for this effect was weak (p = `r zinb.comparison %>% filter(Component=="Binary" & Predictor=="Elevation") %>% pull(p)`; Table \@ref(tab:zinbsummary)). In the count component of the model, elevation and the density of conifer seedlings were significantly related to aspen seedling density (p\<0.05; Table \@ref(tab:zinbsummary); Fig. \@ref(fig:FigDensity)B-C). Density increased with elevation (standardized β = `r zinb.coef %>% filter(Component=="Count" & Predictor=="Elevation") %>% pull(Estimate)`; standard error = `r zinb.coef %>% filter(Component=="Count" & Predictor=="Elevation")%>% pull(4)`; Fig. \@ref(fig:FigDensity)B). Aspen seedling density also increased with the natural log of conifer seedling density (standardized β = `r zinb.coef %>% filter(Component=="Count" & Predictor=="Conifer seedling density") %>% pull(Estimate)`; standard error = `r zinb.coef %>% filter(Component=="Count" & Predictor=="Conifer seedling density") %>% pull(4)`; Fig. \@ref(fig:FigDensity)C).

```{r zinbsummary, tab.cap='Summary of zero-inflated negative binomial model selection results for absence (binary model) and abundance (count model) of post fire aspen stems. χ2 test statistics and p-values are from likelihood ratio tests comparison the full model to a reduced model where the focal predictor is dropped. AIC values are for the reduced model (i.e., higher AIC indicates removing this variable reduces model fit).', tab.id='zinbsummary'}
zinb.comparison  %>% 
...
Within sites with aspen seedlings, we found microsite conditions were important predictors of aspen seedling establishment (Fig. \@ref(fig:FigMicro)). Notably, aspen seedlings were about `r (perf.dat %>% filter(level=="bryophytes") %>% pull(estimate)) %>% round(1)` times more likely to occur on bryophytes than any other substrate type. Seedlings also preferentially established in large and small concavities, with preferences of `r (perf.dat %>% filter(level=="concave" & variable=="Large topography") %>% pull(estimate)) %>% round(1)` and `r (perf.dat %>% filter(level=="concave" & variable=="Small topography") %>% pull(estimate)) %>% round(1)`, respectively. Finally, aspen seedlings were more likely to establish in microsites near large CWD than microsites with small CWD, small and large CWD, or no CWD (preference for large CWD = `r perf.dat %>% filter(variable=="CWD" & level=="large") %>% pull(estimate) %>% round(1)`)
...
Two-years following fire, we found limited conifer regeneration but widespread establishment of aspen seedlings across conifer forests that burned at high severity during the Cameron Peak Fire, despite the absence of aspen in the pre-fire stand. Aspen seedlings were more likely to occur at higher elevations, where they were often clustered within concave microsites, on bryophyte substrates, and near large CWD. Collectively, this adds to the support for the idea that cooler and wetter climate conditions promote aspen seedling establishment in recently burned ecosystems [@quinn2001QuakingAspenReproduce; @buma2012DifferentialSpeciesResponses; @kreider2021LowdensityAspenSeedling; @kreider2021AspenSeedlingEstablishment]. We also found aspen seedlings were more likely to occur at sites with greater postfire conifer seedling densities, consistent with previous research which has highlighted the potential for aspen to expand into recently burned conifer forests and shape the dynamics of burned ecosystems [@gill2017PopulusTremuloidesSeedling; @nigro2022WildfireCatalyzesUpward].

## Distribution and abundance of aspen and conifer seedlings

Across the Cameron Peak Fire, we found aspen seedlings at `r (seedlingdensity.tph %>% filter(POTR5.seedling>0) %>% nrow() / nrow(seedlingdensity.tph) *100) %>% round(0)` % of sites surveyed. This occupancy rate is identical to the rate reported by Kreider and Yocom [-@kreider2021AspenSeedlingEstablishment] for the Brian Head fire in Utah and similar to the highest fire-level occupancy rates reported by Kreider and Yocom [-@kreider2021LowdensityAspenSeedling] for 15 fires spanning a latitudinal gradient from northern Arizona to northern Wyoming. In Kreider and Yocom's [-@kreider2021LowdensityAspenSeedling] West-wide study, they reported aspen seedlings in only 13 % of sites, with occupancy rate for individual fires events varying from 0 to 61 % [@kreider2021LowdensityAspenSeedling]. In the case of the Cameron Peak Fire, the high occupancy rates we report here may be attributed to our sampling design, which included larger sampling area than Kreider and Yocom [-@kreider2021LowdensityAspenSeedling] and excluded areas that burned at low or moderate severity, where seedling establishment may be less likely [@turner2003PostfireAspenSeedling; @gill2017PopulusTremuloidesSeedling]. When considering only one transect per site, we found seedlings present at `r (seedlingdensity.dat %>% filter(Transect=="A") %>% group_by(Site) %>% summarize(occupied=ifelse(sum(POTR5.seedling, na.rm=T)>0, 1, 0)) %>% pull(occupied) %>% sum() / length(unique(seedlingdensity.dat$Site))*100) %>% round()` % per site, which suggests that the larger sample area is not responsible for the difference. Further, across the 10 fires with sites that burned at high severity [defined as dNBR \>418 following Parks et al. [-@parks2018MeanCompositeFire]] in Kreider and Yocom's [-@kreider2021LowdensityAspenSeedling] West-wide study, aspen seedlings were detected at 0 to 64% of the severely burned sites within a fire event (median = 10%) [@kreider2021MarkkreiderAspenSeedlingFireSurveyFirst]. Thus, the high occupancy rates we report here likely reflect characteristics of the post-fire environment, such as climate and landscape-level patterns of aspen seed production. For example, all our study sites were located at higher elevations than the highest elevation site sampled by Kreider and Yocom [-@kreider2021LowdensityAspenSeedling]. The relatively cool and wet site conditions sampled herein may drive the higher establishment rates.

At sites with aspen seedlings, our median density of `r seedlingdensity.plot %>% filter(POTR5.seedling.density>0) %>% pull(POTR5.seedling.density) %>% median()` seedlings ha^-1^ and maximum density of `r seedlingdensity.plot %>% pull(POTR5.seedling.density) %>% max()` seedlings ha^-1^ is comparable to median (range: 100-900; median across fires: 100 seedlings per ha^-1^) and maximum (range: 100-9700; median across fires: 150 seedlings ha^-1^) densities for the 15 fires surveyed by Kreider and Yocom [-@kreider2021LowdensityAspenSeedling]. However, postfire establishment of aspen from seed can be extremely abundant. For example, following both the 2017 Brian Head and the 1998 Yellowstone fires aspen, seedling densities one-year following fire exceeded 100,000 seedlings ha^-1^ [@kay1993AspenSeedingsRecently; @kreider2021AspenSeedlingEstablishment]. However, survival of aspen seedlings is often low in the first year and such high densities are unlikely to persist for multiple years [@romme2005EstablishmentPersistenceGrowth; @kreider2021AspenSeedlingEstablishment]. Because we sampled two years following the Cameron Peak fire, our sample consists of both first- and second- year aspen seedling germinates (i.e. seedlings that established in 2021 and 2022), if most establishment occurred one-year following fire than we may have missed peak densities. Nonetheless, within the Cameron Peak fire only `r (compiled.site.plot %>% filter(POTR5.seedling>741) %>% nrow() / compiled.site.plot %>% filter(POTR5.seedling>0) %>% nrow()*100) %>% round(0)` % (n= `r compiled.site.plot %>% filter(POTR5.seedling>741) %>% nrow()`) of the sites with aspen seedlings surveyed here would meet local USFS requirements for aspen restocking (741 seedlings greater than one year in age per ha^-1^) [@usfs19971997REVISIONLAND]. Thus, if the aspen seedlings survive in this landscape, many of these areas may transition to low density aspen woodlands rather than aspen forests without continued infilling.

Two years following fire, we found relatively low regeneration of subalpine fir, Engelmann spruce and ponderosa pine, and to a lesser extent lodgepole pine despite their presence in the pre-fire community. Compared to local USFS requirements for conifer restocking (370 seedlings per ha^-1^) [@usfs19971997REVISIONLAND], only six of our sites were stocked - five sites with lodgepole pine and one site where densities of both lodgepole pine and Engelmann spruce exceeded the stocking threshold. These low densities may have been due to the relatively warm and dry weather in the two years post-fire. Such conditions have emerged as important constraints on postfire germination and initial survival of conifer seedlings in recent decades [@stevens-rumann2019TreeRegenerationFollowing]. Additionally, viable seed may have been limited by pre-fire outbreaks of bark beetles [@rhoades2022LimitedSeedViability], which caused widespread and severe mortality of lodgepole pine and Engelmann spruce across the study area in the beginning of the 21st century [@rodman2021EffectsBarkBeetle]. In the case of lodgepole pine, the extreme fire behavior that characterized by much of the area burned in Cameron Peak fire also likely resulted consumption of much of the canopy seedbank [@rhoades2025PostfireSeedlingRecruitment]. Collectively, the post-fire weather conditions, pre-fire bark beetle outbreaks, and extreme nature of the fire event set the stage for low conifer regeneration. While infilling of burned areas with conifers seedlings can continue for decades following the fire, seedling densities two-years following fire are often good predictors of recruitment failure in conifer forests across the West [@stevens-rumann2018EvidenceDecliningForest]. Thus, in the absence of human intervention, conifers may remain limited across the system. Nonetheless, further research is needed to understand how the forests may develop [@gill2017InitialPostdisturbanceRegeneration].

While aspen seedling regeneration was not incredibly abundant across the Cameron Peak burn scar, densities of aspen seedlings often exceeded densities of conifer seedlings. Here we found aspen seedlings were present at `r seedlingdensity.tph %>% filter(Conifer >0 & POTR5.seedling>0) %>% nrow() / seedlingdensity.tph %>% filter(Conifer >0) %>% nrow() *100 %>% round()` % of the sites with conifer seedlings and outnumbered conifer seedlings at `r (seedlingdensity.tph %>% filter(Conifer >0 & POTR5.seedling>0) %>% filter(POTR5>Conifer) %>% nrow() / seedlingdensity.tph %>% filter(Conifer >0 & POTR5.seedling>0)  %>% nrow() *100) %>% round()` % of these sites. Further aspen seedlings and conifer seedlings were more commonly found clustered together within a site than expected by complete spatial randomness. The co-occurrence of aspen and conifer seedlings both across and within sites suggests rates and trajectories of post-fire stand development will likely lead to competition between aspen and conifers. This competition along with species-level difference in functional traits (e.g., resprouting) and vital rates (e.g., growth, survival) will drive post-fire forest dynamics.

## Drivers of aspen seedling establishment

Across our study sites, aspen seedling occurrence and density increased with elevation. Higher elevations are characterized by cooler summers and greater winter snowpack accumulation and retention, which lower moisture stress. Low elevation sites, in addition to already being at the lower bounds of aspen habitat suitability [@rehfeldt2009AspenClimateSudden], were also characterized by greater herbaceous cover, which may both out compete aspen seedlings and occupy the substrate preferred by seedlings [@kreider2021LowdensityAspenSeedling]. Nonetheless, we observed aspen seedlings as low as `r compiled.site.plot %>% filter(POTR5.seedling>0) %>% pull(Elevation) %>% min()` m, where annual precipitation averages `r compiled.site.plot %>% filter(Elevation==compiled.site.plot %>% filter(POTR5.seedling>0) %>% pull(Elevation) %>% min()) %>% pull(ppt) %>% round()` mm and mean daily temperature averages `r compiled.site.plot %>% filter(Elevation==compiled.site.plot %>% filter(POTR5.seedling>0) %>% pull(Elevation) %>% min()) %>% pull(tmean) %>% round(1)`°C. In comparison, Kreider and Yocom [-@kreider2021LowdensityAspenSeedling] reported postfire aspen seedling establishment was unlikely where average annual precipitation values were less than 750 mm on average. Yet here we found `r compiled.site.plot %>% mutate(low=ifelse(ppt<775, "low", "high")) %>% filter(POTR5.seedling>0)  %>% group_by(low) %>% summarize(mean=mean(POTR5.seedling), median=median(POTR5.seedling), n=n()) %>% mutate(percent=round(n/sum(n)*100)) %>% filter(low=="low") %>% pull(percent)` % of sites with aspen seedlings normally received less than 750 mm precipitation annually. These differences may be due regional adaptations to moisture availability or strong effects of microsite conditions on soil moisture.

Contrary to previous research which clearly shows that aspen seedling density is greater when live aspen trees are nearby [@gill2017PopulusTremuloidesSeedling; @kreider2021AspenSeedlingEstablishment; @kreider2021LowdensityAspenSeedling], our modeling did not support this expectation. However, we focused on understanding the potential for fire to expand (not maintain) aspen populations and only sampled sites at least 50 meters away from an aspen that was live prior to the fire. Consequently, our sampling design cannot test for effects of fine-scale seed dispersal. Nonetheless, previous research suggestions that distance to seed source can positively influence seedling establishment at distances well beyond a kilometer [@gill2017PopulusTremuloidesSeedling] and we did not detect this effect. In the case of the Cameron Peak fire, the distance from live aspen generally increased with elevation, largely because aspen were more common at lower elevations prior to the fire and fire severity increased with elevation (Fig. **S**\@ref(fig:FigElevationxDist)). As result, sites with higher climatic suitability were typically at a greater distance from live seed sources. Such mismatches between landscape-patterns of climatic suitability and seed availability may limit the potential for aspen to expand into new habitats.

At the microsite scale, seedling occurrence was related to bryophyte cover, presence of CWD, and concave microtopography, which collectively have connections to the accumulation and retention of seeds, moisture, and nutrients [@kreider2021AspenSeedlingEstablishment; @fairweather2014AspenSeedlingEstablishment]. Fallen logs or other large debris and local concavities may serve to collect both seeds and moisture necessary for successful establishment and survival in the first 1-3 growing seasons [@fairweather2014AspenSeedlingEstablishment]. Similarly bryophytes can facilitate seedling establishment by mediating temperature and moisture conditions and supplying nutrients [@chapin1987RoleMossesPhosphorus; @wheeler2011FeathermossSeedbedsFacilitate; @mallik2018LichenMattedSeedbeds]. Importantly, microsite conditions favorable to aspen establishment were not equally abundant or evenly distributed across the fire. For example, we identified bryophytes as the dominant substrate at `r  sitemicro.dat %>% group_by(Substrate) %>% summarize(n=n()) %>% mutate(freq=round(n/sum(n)*100,0)) %>% filter(Substrate=="bryophytes") %>% pull(freq)` % of systematic samples, while we found `r seedlingmicro.dat %>% group_by(Substrate) %>% summarize(n=n()) %>% mutate(freq=round(n/sum(n)*100,0)) %>% filter(Substrate=="bryophytes") %>% pull(freq)` % of aspen seedlings had established on bryophytes. Further, the availability of microsite conditions favorable to aspen seedling establishment generally increased with elevation, yet the proportion of aspen seedlings found within favorable microsite conditions often decreased with elevation (Fig. **S**\@ref(fig:FigCWDXClimate)-S\@ref(fig:FigLTopoXClimate)). While our dataset is not robust enough to adequately test for this interaction, it suggests that microsite conditions may be more important drivers of aspen seedling establishment at harsher sites. Further research is needed to understand how suitable microsites are distributed across landscapes and how the effects of microsite conditions vary across coarser topoclimatic gradients.

# Conclusions

Increased fire frequency, severity, and extent in western North America may exceed the capacity of some formerly conifer-dominant forests to maintain historical distributions [@coop2020WildfireDrivenForestConversion], while conditions following high severity burns may allow establishment of aspen seedlings both within and outside of pre-fire aspen populations [@kreider2021LowdensityAspenSeedling; @nigro2022WildfireCatalyzesUpward]. We observed this phenomenon in high severity zones of the Cameron Peak Fire, particularly in areas where topographic heterogeneity and post-fire legacies resulted in increased moisture availability. Concurrent with aspen seedling regeneration, we found low overall regeneration of conifer species relative to aspen. Across our system, limited conifer regeneration likely results from warm and dry weather and pre-fire outbreaks of bark beetles that caused extensive tree mortality - conditions that are expected to become more common with climate change [@bentz2010ClimateChangeBark; @ipcc2021ClimateChange2021]. These trends coupled with changing fire regimes and our observations, along with a growing body of research on aspen seedling regeneration in the West [e.g., @gill2017PopulusTremuloidesSeedling; @kreider2021LowdensityAspenSeedling; @andrus2021FutureDominanceQuaking; @nigro2022WildfireCatalyzesUpward], suggest that aspen may increasing shape post-fire ecology across the West. Such increases in the proportion of a landscape composed of young post-fire aspen stands may foster greater resistance to wildfire [@harris2025AspenImpedesWildfire], increase biodiversity [@mccullough2013QuantifyingConsequencesConifer], and enhance retention of soil organic carbon [@wang2025SoilSignalsKey], among other things.
...
The datasets generated and/or analyzed during the current study are available in Dryad Research Data Repository, <http://datadryad.org/share/-sMlq6LIsb7GIoGZg4MPGdPu4mz-UeqW61o7YHFLGAM>.
...
compiled.site.plot %>% 
  arrange(Site) %>% 
  dplyr::select(Site, Latitude, Longitude, Elevation, tmean, ppt, distance.pre, distance.post, Dist.Con, POTR5.seedling, Conifer) %>% 
  mutate(Latitude=round(Latitude, 3), Longitude=round(Longitude,3), tmean=round(tmean, 1), ppt=round(ppt,0), Elevation=round(Elevation,0), distance.post=round(distance.post,0),  distance.pre=round(distance.pre,0)) %>% 
  set_colnames(c("Site", "Latitude\n(°N)", "Longitude\n(°W)", "Elevation (m)", "Mean annual temperature\n(°C)", "Mean annual precipitation\n(mm)", "Distance to pre-fire aspen\n(m)", "Distance to post-fire live aspen\n (m)","Distance to post-fire live conifer\n (m)", "Aspen seedling density\n(no. per ha)",  "Conifer seedling density\n(no. per ha)")) %>% 
  qflextable() %>%  
...
```{r FigSubCluster, fig.cap="Boxplots illustration the proportion of subplots with multiple seedlings for sites with seedlings. For each box, the horizontal line is the median value and the green diamond is the mean, while the top and bottom edges of the box  indicate the first and third quartiles. The whiskers extend to the most extreme data points within 1.5 times the interquartile range from the box edges. Individual points displayed beyond the whiskers represent outliers.", fig.cap.pre="Figure S", fig.lp="supp-fig", fig.width=4.5, fig.height=3.5, fig.id="FigSubCluster"}
knitr::include_graphics(here("Results", "Figures", "SubCluster.jpg"))
```

```{r FigSubClusterXDensity, fig.cap="Scatter plots illustrating the relationship between seedling density and the proportion of subplots with multiple seedling", fig.cap.pre="Figure S", fig.lp="supp-fig", fig.width=4.5, fig.height=3.5, fig.id="FigSubClusterXDensity"}
knitr::include_graphics(here("Results", "Figures", "SubClusterXDensity.jpg"))
```
...
```{r FigZinbResidsMoransI, fig.cap="Plots of the simulated residuals from the zero-inflated negative binomial model of aspen seedling density as a function of geographic coordinates. Moran's I test for distance-based autocorrelation shows no significant autocorrelation in the model residuals (I =0.065, E(I) = -0.030, p=0.327).Both the plot and statistics were generated using the DHARMa package (Hartig 2024).", fig.cap.pre="Figure S", fig.lp="supp-fig",fig.width=4, fig.height=4,fig.id="FigZinbResidsxPreds"}
knitr::include_graphics(here("Results", "figures", "zinb-MoransI.jpg"))
```

```{r FigElevationxDist, fig.cap="The relationship between elevation and aspen's distribution across the area burned in the Cameron Peak Fire. A) Boxplots comparing the elevation range of areas with and without aspen prior to the fire. B) Boxplots comparing the elevation by fire severity. C) The density of observations for areas that burned in the Cameron Peak fire that were greater than 50 m from live aspen and less than 1000 km from live aspen prior to the fire by elevation and distance to seed source. For each box, the horizontal line is the median value, while the top and bottom edges of the box  indicate the first and third quartiles. The whiskers extend to the most extreme data points within 1.5 times the interquartile range from the box edges. Individual points displayed beyond the whiskers represent outliers.", fig.lp="supp-fig", fig.cap.pre="Figure S", fig.id="FigElevationxDist", fig.width=7, fig.height=2.5}
...
```{r FigDistCon, fig.cap="The association between conifer seedling density and distance to a live conifer seed source for sites with conifer seedlings. Distance to live conifer was manually recorded using 1-m aerial imagery collected in 2021 by the National Agriculture Imagery Program.", fig.lp="supp-fig", fig.cap.pre="Figure S", fig.id="FigElevationxDist", fig.width=5, fig.height=3}
condist <- compiled.site.plot  %>%  select(Site, Dist.Con, ABLA, PICO, PIEN, PSME, PIPO) %>% pivot_longer(cols=c(ABLA, PICO, PIEN, PSME, PIPO), names_to="Species", values_to="Density") %>% mutate(Species=recode(Species, ABLA="subalpine fir", PICO="lodgepole pine", PIEN="Engelmann spruce", PSME="Douglas fir", PIPO="ponderosa pine")) %>% filter(Density>0)

p1 <- ggplot(condist, aes(x=Dist.Con, y=Density, color=Species))+geom_jitter()+ylab("conifer seedling density\n(no. per ha)")+xlab("distance to live conifer\n(m)")+theme(legend.position = "right")
ggsave(p1, filename=here("Results", "Figures", "FigDistCon.jpg"), width=5, height=3, units="in", dpi=300)
knitr::include_graphics(here("Results", "Figures", "FigDistCon.jpg"))
```

```{r,FigCWDXClimate, fig.cap="Scatterplots illustrating the association between site-level topoclimatic variables and the percentage of aspen seedlings occurring within a given microsite CWD class (green) or the percentage of systematic samples occurring within a given microsite CWD class (gray). The plot shows that both the occurrence of CWD conditions and aspen seedling usage of those conditions vary with site climate conditions. Lines show the linear regressions fit to the data and the shaded areas represents the 95% confidence intervals for the regression lines.", fig.lp="supp-fig", fig.cap.pre="Figure S", fig.id="FigCWDXClimate", fig.width=7, fig.height=5}
x <- seedlingmicro.dat %>% 
  group_by(Site, Combo.CWD) %>% 
  summarize(n=n()) %>% 
  mutate(p=round(n/sum(n)*100), variable="aspen seedling")
y <- sitemicro.dat.sub %>% 
  group_by(Site, Combo.CWD) %>% 
  summarize(n=n()) %>% 
  mutate(p=round(n/sum(n)*100), variable="systematic sample")

xy <- bind_rows(x,y) %>%
  na.omit() %>% 
  left_join(compiled.site.plot, by="Site")
  
p0 <- xy %>% 
  ggplot(aes(x=Elevation, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Combo.CWD, ncol=1) +xlab("elevation (m)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35" ))+scale_fill_manual(values=c("#2ca25f", "grey35"))
p1 <- xy %>% 
  ggplot(aes(x=ppt, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Combo.CWD, ncol=1) +xlab("mean annual total\nprecipitation (mm)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35" ))+scale_fill_manual(values=c("#2ca25f", "grey35"))

p2 <- xy %>% ggplot(aes(x=tmean, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Combo.CWD, ncol=1) +xlab("mean annual daily\ntemperature (ºC)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35"))+scale_fill_manual(values=c("#2ca25f", "grey35"))

p12 <- p0+ p1 + p2 +plot_layout(guides = 'collect', axes = "collect")& 
  theme(legend.position = 'bottom', legend.title=element_blank())
ggsave(p12, filename=here("Results", "Figures", "FigCWDXClimate.jpg"), width=7, height=5, units="in", dpi=300)
knitr::include_graphics(here("Results", "Figures", "FigCWDXClimate.jpg"))
```

```{r FigSTopoXClimate, fig.cap="Scatterplots illustrating the association between site-level topoclimatic variables and the percentage of aspen seedlings occurring within a given small (0-2.5 cm) microtopographic class (green) or the percentage of systematic samples occurring within a given small microtopographic class (gray). The plot shows that both the occurrence of small microtopographic conditions and aspen seedling usage of those conditions vary with site climate conditions. Lines show the linear regressions fit to the data and the shaded areas represents the 95% confidence intervals for the regression lines.", fig.lp="supp-fig", fig.cap.pre="Figure S", fig.id="FigSTopoXClimate", fig.width=7, fig.height=4}

x <- seedlingmicro.dat %>% 
  group_by(Site, Small.Topo) %>% 
  summarize(n=n()) %>% 
  mutate(p=round(n/sum(n)*100), variable="aspen seedling")
y <- sitemicro.dat.sub %>% 
  group_by(Site, Small.Topo) %>% 
...
p0 <- xy %>% ggplot(aes(x=Elevation, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Small.Topo, ncol=1) +xlab("elevation (m)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35" ))+scale_fill_manual(values=c("#2ca25f", "grey35")) 

p1 <- xy %>% ggplot(aes(x=ppt, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Small.Topo, ncol=1)+xlab("mean annual total\nprecipitation (mm)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35" ))+scale_fill_manual(values=c("#2ca25f", "grey35"))

p2 <- xy %>% ggplot(aes(x=tmean, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Small.Topo, ncol=1) +xlab("mean annual daily\ntemperature (ºC)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35"))+scale_fill_manual(values=c("#2ca25f", "grey35"))

p12 <- p0+ p1 + p2 +plot_layout(guides = 'collect', axes = "collect")& theme(legend.position = 'bottom', legend.title=element_blank()) 
ggsave(p12, filename=here("Results", "Figures", "FigSTopoXClimate.jpg"), width=7, height=4, units="in", dpi=300) 
knitr::include_graphics(here("Results", "Figures", "FigSTopoXClimate.jpg"))



```

```{r FigLTopoXClimate, fig.cap="Scatterplots illustrating the association between site-level topoclimatic variables and the percentage of aspen seedlings occurring within a given large (0-10 cm) microtopographic class (green) or the percentage of systematic samples occurring within a given large microtopographic class (gray). The plot shows that both the occurrence of CWD conditions and aspen seedling usage of those conditions vary with site climate conditions. Lines show the linear regressions fit to the data and the shaded areas represents the 95% confidence intervals for the regression lines.", fig.lp="supp-fig", fig.cap.pre="Figure S", fig.id="FigLTopoXClimate", fig.width=7, fig.height=4}

x <- seedlingmicro.dat %>% 
  group_by(Site, Large.Topo) %>% 
  summarize(n=n()) %>% 
  mutate(p=round(n/sum(n)*100), variable="aspen seedling")
y <- sitemicro.dat.sub %>% 
  group_by(Site, Large.Topo) %>% 
...
p0 <- xy %>% ggplot(aes(x=Elevation, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Large.Topo, ncol=1) +xlab("elevation (m)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35" ))+scale_fill_manual(values=c("#2ca25f", "grey35")) 

p1 <- xy %>% ggplot(aes(x=ppt, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Large.Topo, ncol=1) +xlab("mean annual total\nprecipitation (mm)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35" ))+scale_fill_manual(values=c("#2ca25f", "grey35"))

p2 <- xy %>% ggplot(aes(x=tmean, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Large.Topo, ncol=1) +xlab("mean annual daily\ntemperature (ºC)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35"))+scale_fill_manual(values=c("#2ca25f", "grey35"))

p12 <- p0+ p1 + p2 +plot_layout(guides = 'collect', axes = "collect") & theme(legend.position = 'bottom', legend.title=element_blank()) 

ggsave(p12, filename=here("Results", "Figures", "FigLTopoXClimate.jpg"), width=7, height=4, units="in", dpi=300) 

knitr::include_graphics(here("Results", "Figures", "FigLTopoXClimate.jpg"))
```

```{r FigSubstrateXClimate, fig.cap="Scatterplots illustrating the association between site-level topoclimatic variables and the percentage of aspen seedlings occurring within a given microsite substrate class (green) or the percentage of systematic samples occurring within a given microsite substrate class (gray). The plot shows that both the occurrence of substrate conditions and aspen seedling usage of those conditions vary with site climate conditions. Lines show the linear regressions fit to the data and the shaded areas represents the 95% confidence intervals for the regression lines.", fig.lp="supp-fig", fig.cap.pre="Figure S", fig.id="FigSubstrateXClimate", fig.width=7, fig.height=6.5}
x <- seedlingmicro.dat %>% 
...
p0 <- xy %>% 
  ggplot(aes(x=Elevation, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Substrate, ncol=1) +xlab("elevation (m)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35" ))+scale_fill_manual(values=c("#2ca25f", "grey35"))
p1 <- xy %>% 
  ggplot(aes(x=ppt, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Substrate, ncol=1) +xlab("mean annual total\nprecipitation (mm)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35" ))+scale_fill_manual(values=c("#2ca25f", "grey35"))

p2 <- xy %>% ggplot(aes(x=tmean, y=p, col=variable, fill=variable))+geom_smooth(se=T, method="lm")+geom_point()+facet_wrap(.~Substrate, ncol=1) +xlab("mean annual daily\ntemperature (ºC)") +ylab("percent of observations")+coord_cartesian(ylim = c(0,100))+scale_color_manual(values=c("#2ca25f", "grey35"))+scale_fill_manual(values=c("#2ca25f", "grey35"))

p12 <- p0+ p1 + p2 +plot_layout(guides = 'collect', axes = "collect")& 
  theme(legend.position = 'bottom', legend.title=element_blank())
ggsave(p12, filename=here("Results", "Figures", "FigSubstrateXClimate.jpg"), width=7, height=6.5, units="in", dpi=300)
knitr::include_graphics(here("Results", "Figures", "FigSubstrateXClimate.jpg"))
```
