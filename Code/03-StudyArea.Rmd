...
# Read in existing veg (ca. 2016)
  EVT.2016 <- rast(here("Data", "Spatial", "LANDFIRE", "LF2016_EVT_200_CONUS", "Tif", "LC16_EVT_200.tif"))
  activeCat(EVT.2016) <- "EVT_PHYS"
...
CameronPeak.sample.pts <- extract(EVT.2016, CameronPeak.sample.pts, bind=T)
...
colnames(CameronPeak.sample.df) <- c("severity", "aspen", "distance.pre", "distance.post", "distance.high", "EVT", "elevation", "ppt", "ppt.111212345", "ppt.678910", "tmean", "tmin1", "tmax7", "tmax.678910", "ppt.2021", "ppt.111212345.2021", "ppt.678910.2021", "tmax.678910.2021", "ppt.2022", "ppt.111212345.2022", "ppt.678910.2022", "tmax.678910.2022",  "bb.presence", "bb.severity")
...
names(sites.sf) <- c("Site", "Aspect", "Slope","severity",  "aspen", "distance.pre", "distance.post", "distance.high", "elevation", "ppt", "ppt.111212345", "ppt.678910", "tmean", "tmin1", "tmax7", "tmax.678910", "ppt.2021", "ppt.111212345.2021", "ppt.678910.2021", "tmax.678910.2021", "ppt.2022", "ppt.111212345.2022", "ppt.678910.2022", "tmax.678910.2022",  "bb.presence", "bb.severity")

# Calc folded aspect
sites.sf$Longitude <- sites %>%  st_as_sf(coords = c("Easting", "Northing"), crs = "EPSG:32613") %>% st_transform(crs="epsg:4326") %>% st_coordinates() %>% as.data.frame() %>% st_drop_geometry() %>% pull(X)
sites.sf$Latitude <- sites  %>%  st_as_sf(coords = c("Easting", "Northing"), crs = "EPSG:32613") %>% st_transform(crs="epsg:4326") %>% st_coordinates() %>% as.data.frame() %>% st_drop_geometry()  %>% pull(Y)
sites.sf$FoldedAspect <- abs( 180 - abs(sites.sf$Aspect-225))
sites.sf$PotentialDirRad <- -1.467+1.582*cos(DegToRad(sites.sf$Latitude))*cos(DegToRad(sites.sf$Slope))-1.5*cos(sites.sf$FoldedAspect)* sin(DegToRad(sites.sf$Slope))*sin(DegToRad(sites.sf$Latitude))-0.262*sin(DegToRad(sites.sf$Latitude))*sin(DegToRad(sites.sf$Slope))+0.607* sin(sites.sf$FoldedAspect)*sin(DegToRad(sites.sf$Slope))
sites.sf$HeatLoad <- exp(sites.sf$PotentialDirRad )

sites.sf %>% st_as_sf() %>% st_write(here("Data", "Spatial", "sites-attributes.gpkg"), append=F)
